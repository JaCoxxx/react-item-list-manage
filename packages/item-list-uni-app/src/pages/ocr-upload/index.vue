...
import type { BaseOptionGroups, InventoryItem, OcrReceiptResult } from "@/shared/types";
import {
	getDefaultCategory,
	getDefaultLocation,
	getDefaultUnit,
...
async function loadData() {
	try {
		...
		error.value = requestError instanceof Error ? requestError.message : "加载 OCR 依赖数据失败";
	} finally {
		loading.value = false;
	}
}
...
async function submitOcr() {
	...
	try {
		...
		error.value = requestError instanceof Error ? requestError.message : "OCR 识别失败";
	} finally {
...
<template>
	<PageLayout title="OCR 上传" sub-title="上传购物小票图片到百度 OCR，并将识别出的商品导入库存。">
		<template #actions>
			<button class="button-secondary" @click="navigateTo('/pages/overview/index')">功能导航</button>
			<button class="button-primary" @click="submitOcr" :loading="uploading">开始识别</button>
		</template>
		...
		<uni-card title="导入默认配置" sub-title="识别到不存在的商品时，将使用这些默认值创建物品。">
