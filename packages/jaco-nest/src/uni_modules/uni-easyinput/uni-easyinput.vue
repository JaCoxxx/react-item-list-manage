<template>
	<view class="uni-easyinput" :class="{ 'uni-easyinput-error': msg }" :style="boxStyle">
		<view class="uni-easyinput__content" :class="inputContentClass" :style="inputContentStyle">
			<uni-icons v-if="prefixIcon" class="content-clear-icon" :type="prefixIcon" color="var(--jn-color-text-tertiary)" @click="onClickIcon('prefix')" size="22"></uni-icons>
			<slot name="left">
			</slot>
			<!-- #ifdef MP-ALIPAY -->
			...
			<template v-if="type === 'password' && passwordIcon">
				<!-- 开启密码时显示小眼睛 -->
				<uni-icons v-if="isVal" class="content-clear-icon" :class="{ 'is-textarea-icon': type === 'textarea' }" :type="showPassword ? 'eye-slash-filled' : 'eye-filled'" :size="22" :color="focusShow ? primaryColor : 'var(--jn-color-text-tertiary)'" @click="onEyes"></uni-icons>
			</template>
			<template v-if="suffixIcon">
				<uni-icons v-if="suffixIcon" class="content-clear-icon" :type="suffixIcon" color="var(--jn-color-text-tertiary)" @click="onClickIcon('suffix')" size="22"></uni-icons>
			</template>
			<template v-else>
				<uni-icons v-if="clearable && isVal && !disabled && type !== 'textarea'" class="content-clear-icon" :class="{ 'is-textarea-icon': type === 'textarea' }" type="clear" :size="clearSize" :color="msg ? 'var(--jn-color-danger)' : focusShow ? primaryColor : 'var(--jn-color-text-tertiary)'" @click="onClear"></uni-icons>
			</template>
...
<script>
	 ...
	 * @property {String }	placeholderStyle	placeholder的样式(内联样式，字符串)，如"color: var(--jn-color-border)"
	 ...
	 * @property {String}	primaryColor	设置主题色（默认var(--jn-color-primary)）
	...
	export default {
			...
			styles: {
				...
				default () {
					return {
						...
						backgroundColor: 'var(--jn-color-text-inverse)',
						disableColor: 'var(--jn-color-surface-muted)',
						borderColor: 'var(--jn-color-border)'
					};
				}
			},
			errorMessage: {
		...
		computed: {
			...
			boxStyle() {
				return `color:${
					this.inputBorder && this.msg ? 'var(--jn-color-danger)' : this.styles.color
				};`;
			},
			// input 内容的类和样式处理
			inputContentClass() {
			...
			inputContentStyle() {
				...
				const borderColor =
					this.inputBorder && this.msg ? 'var(--jn-color-danger)' : focusColor;
				return obj2strStyle({
					'border-color': borderColor || 'var(--jn-color-border)',
...
<style lang="scss">
	$uni-error: var(--jn-color-danger);
	$uni-border-1: var(--jn-color-border) !default;
	...
	.uni-easyinput__content {
		...
		border-color: var(--jn-color-border);
	...
	.uni-easyinput--border {
		...
		padding: 10px 15px;
		// padding-bottom: 0;
		border-top: 1px var(--jn-color-surface-muted) solid;
	}
	...
	.is-disabled {
		background-color: var(--jn-color-surface-muted);
		color: var(--jn-color-border);
