			...
			themeBgColor() {
				if (this.dark) {
					...
					if (this.backgroundColor) {
						...
						return this.dark ? 'var(--jn-color-text-primary)' : 'var(--jn-color-text-inverse)'
					}
				}
				return this.backgroundColor || 'var(--jn-color-text-inverse)'
			},
			themeColor() {
				if (this.dark) {
					// 默认值
					if (this.color) {
						...
						return this.dark ? 'var(--jn-color-text-inverse)' : 'var(--jn-color-text-primary)'
					}
				}
				return this.color || 'var(--jn-color-text-primary)'
			},
			navbarHeight() {
				// #ifdef MP-WEIXIN
...
<style lang="scss" scoped>
	...
	.uni-navbar--shadow {
		box-shadow: 0 1px 6px var(--jn-color-border);
	}
