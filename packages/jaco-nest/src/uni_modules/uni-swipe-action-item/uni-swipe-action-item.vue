			...
			<!-- #endif -->
				...
				<view class="uni-swipe_button-group button-group--left">
					<slot name="left">
					  ...
					  backgroundColor: item.style && item.style.backgroundColor ? item.style.backgroundColor : 'var(--jn-color-text-tertiary)'
							...
							<text class="uni-swipe_button-text"
								:style="{color: item.style && item.style.color ? item.style.color : 'var(--jn-color-text-inverse)',fontSize: item.style && item.style.fontSize ? item.style.fontSize : '16px'}">{{ item.text }}</text>
						</view>
					</slot>
				</view>
			...
			<view ref='selector-left-button--hock' class="uni-swipe_button-group button-group--left">
				<slot name="left">
				  ...
				  backgroundColor: item.style && item.style.backgroundColor ? item.style.backgroundColor : 'var(--jn-color-text-tertiary)'
						...
						<text class="uni-swipe_button-text"
							:style="{color: item.style && item.style.color ? item.style.color : 'var(--jn-color-text-inverse)', fontSize: item.style && item.style.fontSize ? item.style.fontSize : '16px'}">
							{{ item.text }}
						</text>
					</view>
				</slot>
					...
					<slot name="left">
					  ...
					  fontSize: item.style && item.style.fontSize ? item.style.fontSize : '16px'
							...
							@touchend.stop="appTouchEnd($event,index,item,'left')"><text class="uni-swipe_button-text"
								:style="{color: item.style && item.style.color ? item.style.color : 'var(--jn-color-text-inverse)',}">{{ item.text }}</text>
						</view>
					</slot>
				</view>
				<slot></slot>
...
<style lang="scss">
	...
	.movable-view-button {
		...
		flex-direction: row;
		height: 100%;
		background: var(--jn-color-text-tertiary);
	}
