 ...
 * @Date 2026-04-24 10:36:02
 * @LastEditTime 2026-04-24 10:36:04
 * @LastEditors zhiyuan.wu zhiyuan.wu@hand-china.com
 * @Description 
 */
import { CameraOutlined, RobotOutlined, UploadOutlined } from "@ant-design/icons";
import {
	Alert,
	App as AntdApp,
	Button,
...
function AiReceiptPage({ aiProvider }: AiReceiptPageProps) {
	...
	const [defaultCategoryCode, setDefaultCategoryCode] = useState<string | undefined>(undefined);
	const [defaultUnitCode, setDefaultUnitCode] = useState<string | undefined>(undefined);
	const [defaultLocationCode, setDefaultLocationCode] = useState<string | undefined>(undefined);
	...
	const recognizeReceipt = useCallback(async () => {
		...
		try {
			...
			message.success("AI 识别购物小票完成");
		} catch (requestError) {
			const nextError =
				requestError instanceof Error ? requestError.message : "AI识别失败";
			setOcrError(nextError);
			message.error(nextError);
		} finally {
			setRecognizing(false);
		}
	}, [aiProvider, allItems, message, selectedFile]);
			...
			for (const item of nextItems.filter((entry) => entry.selected)) {
				...
				try {
					...
					const stockInPayload: Record<string, unknown> = {
						...
						note: `AI小票导入：${item.product || "未命名商品"}`,
					};
					if (unitPrice !== null) {
		...
		<div className="page-stack page-shell">
				...
				<div className="page-title-copy">
					<div>
						...
						<Text type="secondary" className="page-lead-text">
							使用 GPT-4o Vision 识别购物小票。识别完成后勾选要入库的商品，未匹配的商品可填写物品名后自动新建。
						</Text>
					</div>
				</div>
			</div>
			...
			<Card className="surface-card">
				<Space direction="vertical" size={12} className="ocr-result-stack">
					...
					{ocrResult ? (
						<>
							{editableFieldLines.length > 0 ? (
								<div className="ocr-defaults-panel">
									<Text type="secondary">识别到的小票信息</Text>
									<div className="ocr-item-edit-grid">
										{editableFieldLines.map((line) => (
											<Input
												key={line.id}
												addonBefore={line.label}
												value={line.value}
												onChange={(event) =>
													setEditableFieldLines((prev) =>
														prev.map((f) =>
															f.id === line.id ? { ...f, value: event.target.value } : f
														)
													)
												}
											/>
									...
									{editableItemLines.map((item, index) => (
										<div className="ocr-item-edit-row" key={item.id}>
											<div className="ocr-item-row-head">
												...
												{item.importMessage ? (
													<Text
														className={`ocr-item-status is-${item.importStatus ?? "skipped"}`}
													>
														{item.importMessage}
													</Text>
												) : null}
						...
						<Text type="secondary">AI 识别完成后，这里会展示购买明细。</Text>
					)}
				</Space>
			</Card>
		</div>
